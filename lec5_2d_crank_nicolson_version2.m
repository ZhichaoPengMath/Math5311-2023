...
function error_inf=lec5_2d_crank_nicolson_version2(Nx,Ny,sigma0,T)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Lx = zeros(tot_dof,tot_dof);
Ly = zeros(tot_dof,tot_dof);

for i = 1:Nx-1
    for j = 1:Ny-1
        Lx( ind_map(i,j),ind_map(i,j) ) = -2;
        Ly( ind_map(i,j),ind_map(i,j) ) = -2;
        
        if (i>1)
            Lx( ind_map(i,j),ind_map(i-1,j) ) = 1;
        end
        ...
        if (j>1)
            Ly( ind_map(i,j),ind_map(i,j-1) ) =  1;
        end
...
Identity = speye( (Nx-1)*(Ny-1) );
Lx = sparse(Lx);
Ly = sparse(Ly);%

% (Id - 0.5(rx Lx +ry Ly) ) U^{n+1} =(Id + 0.5(rx Lx +ry Ly) ) U^{n}
A_implicit = Identity-0.5*(rx*Lx+ry*Ly);
A_explicit = Identity+0.5*(rx*Lx+ry*Ly);

% ADI (I-0.5rx Lx) U^{n+1/2} = (I + 0.5 ry Ly) U^n
%     (I-0.5ry Ly) U^{n+1} = (I + 0.5 rx Lx) U^{n+1/2}
