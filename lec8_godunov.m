...
% Run lec8_godunov(600,0.1,0.075,2);
% Run lec8_godunov(300,0.5,0.75,1);
% Run lec8_godunov(600,0.5,0.75,1);

% flux_option = 1: Godunov
% flux_option = 2: Central
function lec8_godunov(N,sigma,T,flux_option)
global dx
global dt
...
for time_step = 1:N_time_step
    ...
    flux_right = 0; 
    for i = 1:N
        % left flux of the element i was the righ flux for the element i-1
        flux_left = flux_right;
        if (i<N)
            flux_right = numerical_flux(u_old(i),u_old(i+1),flux_option); 
        else
            flux_right = numerical_flux(u_old(i),0,flux_option); 
        end
        %flux_right
        u(i) = u_old(i) - dt/dx*(flux_right-flux_left);
