...
% Run lec7_lax_wendroff(100,0.75,2.5);
% Run lec7_lax_wendroff(1000,0.75,2.5);

function lec7_lax_wendroff(N,sigma,T)
...
for time_step = 1:N_time_step
    ...
    u(ind_interior) = u_old(ind_interior)...
              -0.5*sigma*(u_old(ind_interior+1)-u_old(ind_interior-1))...
              +0.5*sigma^2*(u_old(ind_interior+1)-2*u_old(ind_interior)+u_old(ind_interior-1));
